...
\item Single Node Implementation/Optimizations
    ...
    continue to be bound by the memory bandwidth and heavily
...
%% 
    ...
    for {\it{j}}$\in$[1..{\it{n}}] and {\it{l}}$\in$[1..{\it{k}}].
     ...
     \vspace*{0.1in}
     {\it{Exploiting SIMD}}: Refer to Fig.~\ref{fig:ffds?}. 
    ...
    %% 
     ...
     xmm\_a = {\it{vec\_load\_and\_splat}}($A_{{\it{i}},{\it{j}}}$); \\
    ...
    scenarios.
     ...
     \vspace*{0.1in}
     {\it{Exploiting multiple cores}}: As explained above, we
     ...
     \vspace*{0.1in}
     {\it{Cache Blocking}}: For smaller values of {\it{n}}, our
     ...
     \vspace*{0.1in}
     {\it{Multi-socket Optimization}}: Current CPU dies have more than
     one socket~\cite{fds}.
