...
\begin{algorithm}[tb]
 \caption{{\sc RandomizedSVD} Algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \State Initialize $Q \in \reals^{d\times r}$ by sampling $Q_{ij} \sim \mathcal{N}(0, 1)$.

    \For{$q$ times}
        \State $Q \gets \Call{ApplyGramian}{A, Q}$ \Comment{Parallel}
        % TODO: we only keep Q and discard R
        \State $Q \gets \Call{ThinQR}{Q}$
    \EndFor

    \State Let $C = \Call{DistMultiply}{A, Q^T}$. \Comment{Parallel}
