...
%%
    ...
    %% 
     ...
     We now describe the vectorization for computing $X =$ $A^TC$.
...
%%
     ...
     proofs our implementation with respect to increasing number of cores on a
...
dividing the rows between the computational nodes such that each node 
operates on the same number of non zeros. 
...
process. In the first step, we divide the number of rows equally
between the number of nodes, and each node reads in the corresponding
...
and distributes the relevant rows so that each node has rows whose
count of non zero elements is similar. The amount of data transferred
between nodes is only a small fraction of the total input size
...
There is an explicit barrier at the end of each of the {\it{q}}
...
sizes).
%%%
In practice, we achieved near-linear scaling for our test systems with
less than 50 nodes.
