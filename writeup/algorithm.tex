...
% The algorithms for computing the SVD/PCA of $A$ can be classified as either direct 
% or iterative. Direct methods, or transformation methods, repeatedly apply transformations to $A$ to 
% reduce it to an equivalent matrix of a more simple form whose SVD can be determined quickly. Iterative
% methods instead work by repeatedly refining increasingly accurate approximations to the singular values
% and singular vectors, and can be terminated whenever the approximations are acceptably accurate. To compute
% our PCA decompositions, we use the Implicitly Restarted Arnoldi Method, an iterative method introduced in~. The advantage
% of this method is that it does not require access to the entries of $A$, instead it requires only the ability to 
% compute the application of $A$ to any vector.
...
% $\bigO(mnk)$, and $\bigO(k)$ passes over the dataset are needed.  This can be
% expensive when dealing with datasets of even moderately-large
